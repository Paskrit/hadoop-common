...
public class SLSRunner {
    ...
    public SLSRunner(boolean isSLS, String inputTraces[], String nodeFile,
                     ...
                     boolean printsimulation, long scaleFactor, Configuration hdfsConf)
    ...
    private void startNM(Clock clock) throws YarnException, IOException {
        ...
        if (nodeFile.isEmpty()) {
            ...
            if (hdfsConf != null) {
                nodeSet.addAll(SLSUtils.parseNodesFromNodeFile(nodeFile, hdfsConf, LOG));
            } else {
                nodeSet.addAll(SLSUtils.parseNodesFromNodeFile(nodeFile, null, LOG));
            }
        }
    ...
    private void startAM(Clock clock) throws YarnException, IOException {
        ...
        if (isSLS) {
            if (hdfsConf != null) {
                startAMFromSLSTracesOnHDFS(clock, containerResource, heartbeatInterval);
            } else {
    ...
    private void startAMFromSLSTracesOnHDFS(Clock clock, Resource containerResource,
                                            int heartbeatInterval) throws IOException {
        // parse from sls traces
        JsonFactory jsonF = new JsonFactory();
        ObjectMapper mapper = new ObjectMapper();

        String inputTrace = inputTraces[0];

        FileSystem fs = FileSystem.get(hdfsConf);
        Path inputFile = new Path(inputTrace);

        try (BufferedReader reader = new BufferedReader(new InputStreamReader(fs.open(inputFile)))) {
            Iterator<Map> i = mapper.readValues(jsonF.createJsonParser(reader),
                    Map.class);
            while (i.hasNext()) {
                Map jsonJob = i.next();
    ...
    public static void main(String args[]) throws Exception {
        ...
        SLSRunner sls = new SLSRunner(isSLS, inputFiles, nodeFile, output,
                trackedJobSet, cmd.hasOption("printsimulation"), scaleFactor, null);
        sls.start();
